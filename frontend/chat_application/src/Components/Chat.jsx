...
//         try {
//           const userResponse = await axios.get('https://realtimechat-backend-bu2v.onrender.com/api/messages/me');
...
//             const newSocket = io('https://realtimechat-backend-bu2v.onrender.com', {
...
//             // Fetch users
//             try {
//               const usersResponse = await axios.get('https://realtimechat-backend-bu2v.onrender.com/api/messages/users');
...
//           const response = await axios.get(`https://realtimechat-backend-bu2v.onrender.com/api/messages/${selectedUser._id}`);
...
//       const response = await axios.post('https://realtimechat-backend-bu2v.onrender.com/api/messages', messageData);
...
const Chat = () => {
      ...
      try {
        ...
        try {
          const userResponse = await axios.get(
            "https://realtimechat-backend-bu2v.onrender.com/api/messages/me"
          );
          console.log("Current user response:", userResponse.data);
          ...
          if (userResponse.data.user) {
            ...
            // Initialize socket connection after getting user
            const newSocket = io(
              "https://realtimechat-backend-bu2v.onrender.com",
              {
                withCredentials: true,
                transports: ["websocket", "polling"],
              }
            ...
            try {
              const usersResponse = await axios.get(
                "https://realtimechat-backend-bu2v.onrender.com/api/messages/users"
              );
              console.log("Users response:", usersResponse.data);
      ...
      const handleReceiveMessage = (messageData) => {
        ...
        if (
          (messageData.sender._id === selectedUser?._id &&
            messageData.receiver._id === currentUser._id) ||
          (messageData.receiver._id === selectedUser?._id &&
            messageData.sender._id === currentUser._id)
        ) {
          setMessages((prev) => {
            // Check if message already exists to avoid duplicates
            const exists = prev.some((msg) => msg._id === messageData._id);
      ...
      if (selectedUser && currentUser) {
        try {
          ...
          const response = await axios.get(
            `https://realtimechat-backend-bu2v.onrender.com/api/messages/${selectedUser._id}`
          );
          console.log("Messages response:", response.data);
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://realtimechat-backend-bu2v.onrender.com/api/messages",
        messageData
      );
      console.log("Message sent response:", response.data);
  ...
  const getLastMessage = (user) => {
    const userMessages = messages.filter(
      (msg) => msg.sender?._id === user._id || msg.receiver?._id === user._id
    );
    return userMessages[userMessages.length - 1];
  };
    ...
    <div className="flex h-screen bg-gray-900 text-white overflow-hidden">
      ...
      <div
        ...
        ${
          isMobileView
            ? "w-full"
            : "w-full md:w-1/3 lg:w-1/4 min-w-0 flex-shrink-0"
        }
        bg-gray-800 border-r border-gray-700 flex flex-col
        ${!isMobileView && !showSidebar ? "hidden" : ""}
      `}
              ...
              return (
                ...
                >
                  <div className="relative flex-shrink-0">
                    ...
                    <div
                      className={`absolute bottom-0 right-0 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full border-2 border-gray-800 ${
