...
function Signup() {
  ...
  const handleDeleteAccount = async (e) => {
    ...
    try {
      await axios.delete(
        "https://realtimechat-backend-bu2v.onrender.com/api/auth/delete",
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.username || !formData.email || !formData.password) {
      setLoading(false);
      return toast.error("Error : Please fill in all the fields", {
        autoClose: 1500,
      });
    }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    ...
    try {
      const response = await axios.post(
        "https://realtimechat-backend-bu2v.onrender.com/api/auth/signup",
        formData
      );
